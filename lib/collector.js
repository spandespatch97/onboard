...
function getRepoIntel(cwd) {
  try {
    ...
    const testGaps = JSON.parse(binary.runAnalyzer(['repo-intel', 'query', 'test-gaps', '--top', '10', '--map-file', mapFile, cwd]));
    const docDrift = JSON.parse(binary.runAnalyzer(['repo-intel', 'query', 'doc-drift', '--top', '10', '--map-file', mapFile, cwd]));

    return { onboard, canHelp, hotspots, testGaps, docDrift };
  } catch {
    return null;
  }
}
